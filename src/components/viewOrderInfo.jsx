...
export default function ViewOrderInfo(props){
    ...
    const [isModalOpen, setIsModalOpen] = useState(false); //check modal(pop-up window) open close state
    const [notes, setNotes] = useState(order.notes); //defualt notes from order display in notes text area(order.notes)
    ...
    return(
        <>
    <Modal //modal component eka(pop-up window)
    isOpen={isModalOpen}
    ...
    >
        <div className="flex flex-col h-full max-h-[90vh]">
					...
					<div className="px-6 py-4 space-y-6 overflow-y-auto">
						...
						<div className="border border-secondary/10 rounded-xl p-4 bg-white/60">
							...
							<textarea
								...
								value={notes}
								onChange={(e) => {
									if (e.target.value == "") { //there having a problem in text area when type and clear the text the save button still appera, that beacuse after eneter text it will create an empty array , 
										setNotes(null);         //so that empty array is not equal to original notes which is null, so to avoid that we set notes to null when the text area is empty, null means there is not having notes to save ,so the save button will dissapear, these type solutions we called as patch solutions
									} else {
					...
					<div className="px-6 py-4 border-t border-secondary/10 flex justify-end gap-2">
						{(order.notes != notes || order.status != status) && (  //logic ekak liyaganna onenam sagala varahanakin cover kereganna(Save Changes button ekata appear wenne change ekak keroth witheray), and 
                                                                                //order.notes != notes (original order eke notes eka new notes ekata equal naththam) or order.status != status (original order eke status eka new status ekata equal naththam)
							<button
								onClick={() => {
    ...
    <button
        ...
        className="px-4 py-2 bg-accent text-primary rounded-lg hover:bg-accent/80">View</button>

        </>
    )
}

//Chatgpt generated code inside <Modal> </Modal>